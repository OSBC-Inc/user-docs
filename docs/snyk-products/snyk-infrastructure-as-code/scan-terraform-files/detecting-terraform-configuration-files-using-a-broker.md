# Broker를 사용하여 Terraform 구성 파일 탐지

비공개로 호스팅되는 Git 저장소를 사용하는 경우 Snyk Broker를 사용하여 저장소에 Snyk을 연결할 수 있습니다. 자세한 내용은 [설정을 위한 전체 brocker 문서](../../../features/integrations/snyk-broker/set-up-snyk-broker.md)를 참조하십시오. 다음은 Terraform 파일에 필요한 추가 구성에 대해 자세히 설명합니다.
...
3. [Broker 구성](detecting-terraform-configuration-files-using-a-broker.md#undefined) 지침을 따릅니다.
...
Broker는 ACCEPT 환경 변수에서 accept.json 파일(위의 규칙이 추가됨)에 대한 경로를 사용합니다. 아래에서 GitHub Broker에 전달한 예를 볼 수 있습니다.
...
**Note**: 이를 통해 Snyk은 모든 .tf 파일을 쿼리할 수 있습니다. 보다 정확하게 하려면 위의 예제의 경로를 특정 프로젝트나 파일 레이아웃으로 더 제한하도록 변경할 수 있습니다.
