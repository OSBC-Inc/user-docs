...
다음 표는 언어별로 제공되는 일반적인 기능에 대한 개요를 제공합니다. 이러한 기능 외에도 관련된 추가 기능을 제공합니다.
...
Snyk이 디펜던시를 분석하고 구축하는 방식은 프로젝트의 언어 및 패키지 매니저에 따라 다릅니다.

다음과 같이 Java 프로젝트에 툴을 사용하는 법을 안내합니다.
...
* 테스트 진행 이전에 Snyk에서 지원하는 관련 매니페스트 파일을 포함해야 합니다.
...
| `--all-projects`              | monorepos에 사용됩니다. 지원되는 모든 매니페스트가 감지됩니다. Gradle monorepos의 경우 Snyk은 root의 **build.gradle / build.gradle.kts** 파일만 찾고 이후 동일한 로직을 적용합니다. `--all-sub-projects`는 monorepo의 root에서 실행되도록 설계되었습니다. |
...
기본적으로 Snyk CLI는 현재 프로젝트(현재 폴더의 root에 있는 프로젝트) 또는 `--file=path/to/build.gradle`로 지정된 프로젝트만 스캔합니다.

*   모든 프로젝트를 한번에 스캔하려면(권장) `--all-sub-projects` 옵션을 사용하세요.
...
특정 구성을 테스트하기 위해 다음과 같이 진행합니다.

* `--configuration-matching` 파라미터로 [Java 정규식](https://docs.oracle.com/javase/tutorial/essential/regex/)(대소문자 구분 안 함)과 함께 옵션을 사용하십시오. 이후 일치하는 구성이 테스트됩니다. 여러 구성이 일치하면 모두 병합되어 함께 해결됩니다. 예:`'^releaseRuntimeClasspath$|^compile$'`
* 다른 하위 프로젝트에 다른 구성이 포함된 경우 각 하위 프로젝트를 개별적으로 스캔합니다.

**--configuration-matching을 사용하는 방법의 예:**

* `--configuration-matching=compile`은 compile, testCompile, compileOnly 등과 일치합니다.
* `--configuration-matching=^compile$`은 compile만 일치합니다.
* `--configuration-matching='^(debug|release)compile$'`은 debugCompile 및 releaseCompile과 일치합니다.
...
*   **디펜던시 구성을 명시적으로 지정합니다**: 특정 구성을 사용하도록 builde.gradle 파일에서 프로젝트 내 디펜던시를 수정합니다.
    ...
    ```
*   **구성 속성 제한**: 명령을 실행할 때 오류가 수신되면 오류는 사용 가능한 속성 값을 나타낼 수 있으며 Gradle의 오류 세부 정보는 어떤 디펜던시 변형이 어떤 속성과 일치하는지도 나타냅니다. 이러한 세부 정보를 사용하여 속성 필터 옵션을 추가합니다.
...
**compileOnly 구성은 더 이상 사용되지 않으며** 프로젝트가 lockfile을 성공적으로 생성하더라도 이 구성을 확인할 수 없으므로 compileOnly 상태는 포함하지 않습니다. 확인 가능한 구성만 디펜던시 그래프를 계산합니다. 이 문제를 해결하려면 **dependencyLocking 로직을 포함하는 build.gradle을 다음 명령으로 업데이트하는 것이 좋습니다.**
...
* `build.gradle`
* `settings.gradle` (특히 패키지 버전을 선택하지 않은 경우)
* 다음 명령어를 통한 출력된 내용
  * `$ snyk test -d`
...
가져올 프로젝트를 선택한 후 `build.gradle` 파일 및 `gradle.lockfile`(선택 사항)을 기반으로 디펜던시 트리를 빌드합니다.
...
Snyk UI에서는 Artifactory for Maven에서 패키지를 확인하려는 mirror 또는 저장소를 지정할 수 있습니다.
