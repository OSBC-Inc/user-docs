...
`snyk-scm-contributors-count` 도구는 속도 제한을 처리하기 위해 시간당 최대 4,500회 호출이라는 엄격한 제한을 적용하고 429의 응답("너무 많은 호출")을 처리하기 위한 추가 규제 메커니즘을 적용합니다.
...
`snyk-scm-contributors-count` 도구는 속도 제한을 처리하기 위해 분당 최대 120의 호출이라는 엄격한 제한을 적용하고 429회의 응답("너무 많은 호출")을 처리하기 위한 추가 규제 메커니즘을 적용합니다.
